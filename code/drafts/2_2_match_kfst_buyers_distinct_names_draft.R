...
# Draft speed-up: fuzzy match each distinct buyer-name problem once.
# Keep match_date because CVR names are filtered by tender publication date.
fuzzy_match_cols <- c(
  "buyer_name_match",
  "buyer_name_broad",
  "buyer_firm_type",
...
# Expand distinct-name fuzzy matches back to every original buyer row.
...
matched <- rbindlist(
  list(
    matched[name_match_method != "fuzzy"],
    fuzzy_row_lookup[
      matched[name_match_method == "fuzzy"],
...
saveRDS(
  buyer_data,
  file.path(clean_data_dir, "clean_buyer_data_kfst_name_matched_distinct_names_draft.rds")
)
saveRDS(
  manual_buyer_name_review,
  file.path(clean_data_dir, "manual_buyer_name_review_kfst_distinct_names_draft.rds")
)
