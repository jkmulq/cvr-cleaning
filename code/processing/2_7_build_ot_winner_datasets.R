...
# Shared slim schema (identical to the KFST stack in 2_1b). dataset first, then entity, then lot context, then raw.
...
# ---- (1) base: the 2_3 + 2_6 matched OT winners, reduced to the slim schema ------------------
base <- copy(base_raw)
...
# candidates = EVERY DK name (no field-CVR gate; that's the whole point of name_only)
...
# name form (clean = strict default; basic/no-spaces/broad looser). Same block as 2_6.
...
name_only <- no[, .(tender_id, lot_id, winner_number, winner_name, winner_country,
  ...
  name_match_step_code = fifelse(!is.na(winner_cvr_final), "CVR from name matching only (field CVR ignored)", NA_character_),
