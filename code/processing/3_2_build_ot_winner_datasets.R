...
#     (field CVR ignored); generated inline (section 2c).
...
match_cache_ver  <- paste0("p3-", key_sig(clean_data_dir))  # p3: extraction/name_match slices now carry full lot/notice metadata (build_lot_ctx)
...
# natively). Built from base_raw so every lot has context, even lots whose production winners were all
# dropped. build_lot_ctx() keeps every column that is constant within a tender-lot and drops
# entity-identity / per-CVR / per-firm columns (OT per-firm lot_amount / bid_* / bidder_* stay
# row-level) -- see its definition in functions.R. Replaces the hand-kept allow-list that silently
# dropped notice-level fields from the non-production slices.
...
#   winner_cvr_final = the name-matched CVR alone (mirror of extraction). Same primitives + thresholds as
#   2_3; runs at script scope (keep_step_matches() uses <<-). Surfaced ONLY via winner_cvr_final; DK-gated;
#   match_date = tender_publications_firstdContractAwardDate (same publication window 2_3 uses).
nmo <- copy(base_raw)[, .(tender_id, lot_id, winner_number, winner_name, winner_country,
...
nmo <- merge(nmo, km, by = "match_row_id", all.x = TRUE)
...
ref_nm   <- name_match[!is.na(winner_cvr_final) & winner_cvr_final != "",
                       .(data_source = "OpenTender", entity = "winner", method = "name_match", tender_id, lot_id, cvr_final = winner_cvr_final)]
...
# 4 CVR-level dedup -> ONE row per distinct (tender_id, lot_id, winner_cvr_final). A CVR is in the
