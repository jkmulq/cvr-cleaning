...
if (!is.null(.hit)) {
  cat("Stack cache HIT -> skipping OT buyer stack build\n")
  .chk <- file.path(clean_data_dir, "checks"); dir.create(.chk, showWarnings = FALSE, recursive = TRUE)
  saveRDS(.hit$extra, file.path(.chk, "predup_cvr_lists_ot_buyer.rds"))
  save_dataset(.hit$output, Sys.getenv("OT_BUYER_STACK_OUT",
    unset = file.path(clean_data_dir, "ot_buyer_datasets_stacked.rds")))
  quit(save = "no")
}
cat("Stack cache MISS -> building OT buyer stack\n")
...
.run_fuzzy <- function(key, ecol, kcol, flcol, step, thr) {
  ...
  keep_step_matches(add_buyer_context_to_matches(accept_fuzzy_match(fc, threshold = thr)))
}
