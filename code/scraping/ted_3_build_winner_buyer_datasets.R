...
# Lot-level context (one row per notice_id + lot).
lot_cols <- intersect(c("notice_id", "lot", "lot_title", "cpv_main", "contract_type",
                        "lot_estimated_value", "lot_awarded_value", "n_tenders_received"),
                      names(lots))
lot_ctx <- unique(lots[, ..lot_cols], by = c("notice_id", "lot"))

# Notice-level context (one row per notice) - amounts, classification, procedure, dates.
notice_cols <- intersect(c("notice_id", "amount_awarded", "amount_estimated", "n_lots",
                          ...
                          "date_award_dispatch", "date_award_publication"), names(nsum))
