...
buyer_data[, `:=`(
  ...
  # Raw ex-ante (estimated) and ex-post (final/awarded) values, kept separate from
  # tender_amount/lot_amount and with NO imputation/fill. TED native currency (not converted).
...
# 7c Lineage dates from the TED notice-date panel (same panel ted_4 uses, keyed by the
#    award notice = TED notice_id). Mirrors the winner join. (Annualisation is done
#    centrally in 4_combine from contract_duration_months.)
