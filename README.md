...
9. Builds `buyer_data_clean` with one row per listed buyer where possible.
10. Creates buyer helper tables and data-quality flags.
...
- `buyer_data`, `single_buyer`, `multiple_buyer_long`, `buyer_data_clean` - Buyer-focused helper tables. `buyer_data_clean` keeps the combined buyer-level output with buyer-count and name-quality flags.
