...
| `flag_cvr_standardised` | `clean_winner_data` | OpenTender expanded winner row | `TRUE` if any CVR standardisation flag is `TRUE`; otherwise `FALSE` | Summarises whether any raw candidate token behind the final row required syntactic cleanup. |
| `flag_winner_cvr_changed` | `clean_winner_data` | OpenTender expanded winner row | `TRUE` if `winner_cvr_clean` differs from `winner_cvr_candidate_original`; otherwise `FALSE` | Example: `DK21479683` changes to `21479683`. For collapsed rows, `winner_cvr_candidate_original` keeps one separated raw token, `winner_cvr_candidate_audit` keeps all raw tokens that collapsed into the cleaned CVR, and `winner_cvr_original` keeps the full OpenTender source string. |
