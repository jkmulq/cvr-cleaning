...
- [ ] Review the consortium-rollout `cvr_number_source` descriptions (added 2026-08-21).
...
- [x] Recover OpenTender extraction failures from the TED XML (added after PI discussion, 2026-08-03).
  ...
  - Reverted (2026-08-28): approach abandoned to keep the OpenTender dataset **distinct** from anything extracted from the TED XML. Both `code/scraping/ted_2_recover_winners.R` and `code/processing/1_2b_recover_ted_winners.R` were **removed**, and the fold-in step is no longer run. TED-XML winners now live only in the standalone TED dataset (`ted_2_extract_party_cvrs.R` → `ted_3_build_winner_buyer_datasets.R`); OpenTender and KFST gain only the lineage **dates** from the `ted_dates_*` chain, not recovered winners.
  - (Historical) The recovery had folded ~2,079 winner rows over 758 lots into `clean_winner_data_ot.rds` (precise tiers `single_lot_notice` + `lot_no_match` only; the `multi_lot_notice_level` tier was already dropped as ~99% duplication). Left here as a record in case a clean TED-award-block rebuild is revisited.
...
- [ ] Document data provenance for the raw inputs (KFST `udbudsdata_kfst.xlsx`, OpenTender CSVs, Virk CVR keys). Ask PI's coauthors where each source comes from and how a replicator obtains access, then add a "Data availability" section to the README.
  - Partial (2026-08-26): the "how a replicator obtains access" half is done — README *Required local inputs* now states the datasets are not publicly distributed and to contact the maintainer. Remaining: document where each raw source comes from (needs the PI's coauthors).
- [x] Make `config.R` set up all output directories on first run, so a fresh clone works out of the box.
  - **Done (2026-08-26):** `config.R` now creates every `dirs` entry except `code` (`raw`, `clean`, `cvr_matching_data`, `intermediates`) plus the TED cache `intermediates/ted/raw_xml`, via `lapply(c(dirs[setdiff(names(dirs), "code")], file.path(dirs$intermediates, "ted", "raw_xml")), dir.create, recursive = TRUE)`. Input dirs are created empty; `run_replication.sh` still verifies the required input files are present. Verified against a fresh, empty data root.