...
- [x] Carry tender-level fields into `clean_winner_data` before creating tender-level flags.
  - Current state: `n_bids_received` and `tender_cancelled` are selected into `clean_winner_data` when single- and multi-winner records are bound.
